  ...
  render() {
    return (
      ...
      {this.props.motorcycles.map((motorcycle) =>
        <Link to={motorcycle.path.alias} className={[containerStyles.container + " teaser"]} key={motorcycle.id} title={'Learn more about the ' + motorcycle.title + '.'}>
          <span className={containerStyles.category + " category"}>{JSON.parse(JSON.stringify(motorcycle.relationships.field_category['0'].name))}</span>
          <h3 className={containerStyles.motorcyclename}>{motorcycle.title}</h3>
          <GatsbyImage image={getImage(motorcycle.relationships.field_images['0'].localFile.childrenImageSharp['0'])} alt={motorcycle.field_images['0'].alt} className={containerStyles.teaserImage} />
